...
const About = () => {
  ...
  />
      ...
      <section className="mx-auto grid max-w-[1280px] gap-12 px-6 py-24 sm:px-10 lg:grid-cols-[0.8fr_1.2fr] lg:px-16 lg:py-32">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-[var(--vanta-accent)]">
            The Vanta idea
          </p>
        ...
        <div className="max-w-2xl text-sm leading-8 text-[var(--vanta-muted)] sm:text-base">
          <p>
      ...
      <section className="mx-auto grid max-w-[1440px] gap-px bg-white/10 px-6 sm:px-10 lg:grid-cols-2 lg:px-16">
        <div className="relative min-h-[500px] overflow-hidden bg-[var(--vanta-soft)]">
          <img
        ...
        <div className="flex min-h-[500px] flex-col justify-between about-panel bg-[var(--vanta-surface)] p-8 sm:p-12 lg:p-16">
          <Sparkles size={20} className="text-[var(--vanta-accent)]" />
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-[var(--vanta-accent)]">
              Our approach
            </p>
            ...
            <p className="mt-7 max-w-lg text-sm leading-7 text-[var(--vanta-muted)]">
      ...
      <section className="mx-auto max-w-[1280px] px-6 py-24 sm:px-10 lg:px-16 lg:py-32">
        <div className="mb-14 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          ...
          <p className="max-w-sm text-sm leading-6 text-[var(--vanta-muted)]">
            Less decoration. Better decisions. Bags that make sense before
            they make a statement.
          </p>
        </div>

        <div className="grid border-t border-[var(--vanta-border)] md:grid-cols-3">
          {values.map((value) => (
            <article
              key={value.number}
              className="border-b border-[var(--vanta-border)] py-9 md:border-b-0 md:border-r border-[var(--vanta-border)] md:px-8 md:first:pl-0 md:last:border-r-0"
            >
              <span className="text-[11px] tracking-[0.25em] text-[var(--vanta-accent)]">
                {value.number}
              </span>
              <h3 className="mt-8 text-xl font-medium">{value.title}</h3>
              <p className="mt-4 max-w-sm text-sm leading-7 text-[var(--vanta-muted)]">
                {value.text}
              </p>
            </article>
          ))}
      ...
      {/* Stats */}
      <section className="border-y border-[var(--vanta-border)] bg-[var(--vanta-surface)]">
        <div className="mx-auto grid max-w-[1280px] sm:grid-cols-3">
          {stats.map(([number, label]) => (
            <div
              key={label}
              className="border-b border-[var(--vanta-border)] px-6 py-10 last:border-b-0 sm:border-b-0 sm:border-r sm:px-10 sm:last:border-r-0 lg:px-16"
            >
              <p className="font-serif text-4xl">{number}</p>
              <p className="mt-2 text-[11px] uppercase tracking-[0.25em] text-[var(--vanta-muted)]">
                {label}
              </p>
            </div>
          ))}
      ...
      <section className="mx-auto max-w-[1280px] px-6 py-24 text-center sm:px-10 lg:px-16 lg:py-32">
        ...
        <p className="mx-auto mt-7 max-w-xl text-sm leading-7 text-[var(--vanta-muted)]">
        ...
        <div className="mt-9 flex flex-wrap justify-center gap-3">
          <Link
            to="/category"
            className="inline-flex items-center gap-3 bg-[var(--vanta-text)] px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--vanta-bg)] transition hover:bg-[var(--vanta-soft)]"
          >
            Shop bags
            <ArrowRight size={15} />
          </Link>
          <Link
            to="/"
            className="inline-flex items-center gap-2 border border-[var(--vanta-border)] px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--vanta-text)] transition hover:border-[var(--vanta-text)]"
          >
            Back home
          </Link>
        </div>
